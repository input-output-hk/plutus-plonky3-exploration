...
def num_batched_polys(proof):
    ...
    ov = proof["opened_values"]
    if "instances" in ov:
        return sum(_batched_polys_in(inst["base_opened_values"]) for inst in ov["instances"])
...
def convert_batch(obj, src_path):
    ...
    # Milestone A scope: no lookups, no preprocessed, no ZK. Fail loudly if the
    # exporter starts producing them so this converter is extended deliberately.
    coms = proof["commitments"]
    assert coms.get("permutation") is None, "lookup permutation commitment not supported yet"
    assert coms.get("random") is None, "ZK random commitment not supported yet"
    assert all(not l for l in proof.get("global_lookup_data", [])), \
        "global lookup data not supported yet"
    ...
    for inst in proof["opened_values"]["instances"]:
        assert not inst["permutation_local"] and not inst["permutation_next"], \
            "permutation openings not supported yet"
...
/// DO NOT EDIT — regenerate with:
///   cargo run --bin export_batch_proof batch_proof.json
